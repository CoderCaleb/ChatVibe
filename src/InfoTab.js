...
import {
  ref, getDatabase, get, update, push,
...
export default function InfoTab({
  ...
  const {
    setShowCodeModal, setShowRemoveModal, names, author, userState,
      ...
      {messages.type !== 'duo' ? (
        <div className="bg-zinc-900 w-full py-6 flex flex-col pl-7 gap-2">
          ...
          <div className="flex flex-col gap-6">
            {names.map((user, index) => <ContactBar index={index} user={user} key={index} />)}
          </div>
        </div>
      ) : (
        <></>
